...
export const CLEMENTE_GUIDELINES = `
...
export type ClementeCollected = {
  productName?: string;
  category?: string;
  brand?: string;
  model?: string;
  compatibility?: string; // testo libero
  expectations?: string;  // testo libero
  technicalSpecs?: string; // testo libero
  color?: string;
  size?: string;
  material?: string;
  quantity?: number;
...
export function classifyCategory(text: string | undefined): string | undefined {
  if (!text) return undefined;
  const t = text.toLowerCase();
  // Sport e Tempo Libero
  if (/\b(spinning|ciclismo|bici|palestra|running|calcio|sport|mtb|trekking|sci)\b/.test(t)) return 'Sport e Tempo Libero';
  // Elettronica
  if (/(laptop|notebook|pc|computer|gaming|stampante|smartphone|tablet|tv|televisore|monitor|router|cuffie|auricolari|fotocamera|reflex|drone)/.test(t)) return 'Elettronica';
  // Elettrodomestici
  if (/(frigorifero|lavatrice|lavastoviglie|forno|microonde|aspirapolvere|robot\s*aspirapolvere)/.test(t)) return 'Elettrodomestici';
  // Casa e Giardino
  if (/(divano|sedia|tavolo|lampada|illuminazione|giardino|barbecue|bbq|utensili|fai\s*da\s*te|sdraio|lettino|spiaggia|ombrellone|arredo\s*esterno)/.test(t)) return 'Casa e Giardino';
  // Auto e Moto
  if (/(auto|moto|pneumatici|gomme|casco|ricambi|accessori\s*auto|catene\s*da\s*neve)/.test(t)) return 'Auto e Moto';
  ...
  if (/(scarpe|maglietta|pantaloni|giacca|felpa|zaino|borsa|cintura|cappello|guanti)/.test(t)) {
    ...
    if (/(spinning|ciclismo|palestra|running|sport)/.test(t)) return 'Sport e Tempo Libero';
    return 'Abbigliamento e Accessori';
  }
  // Strumenti Musicali
  if (/(chitarra|piano|pianoforte|batteria|sintetizzatore|violino|microfono|mixer|scheda\s*audio)/.test(t)) return 'Strumenti Musicali';
  // Informatica (sottocategoria, ma mappata su Elettronica)
  if (/(ssd|hard\s*disk|scheda\s*video|ram|processore|mouse|tastiera)/.test(t)) return 'Elettronica';
  return undefined;
}

// Estrazione euristica leggera da testo naturale (italiano)
export function extractFromText(text: string): Partial<ClementeCollected> {
  const t = (text || '').toLowerCase();
  const out: Partial<ClementeCollected> = {};

  // Prodotto / categoria (semplici pattern)
  const prodMatch = text.match(/(?:cerco|sto cercando|mi serve|vorrei)\s+([^\.,\n]{3,60})/i);
  if (prodMatch) out.productName = prodMatch[1].trim();
  ...
  if (!out.productName) {
    ...
    if (isShort && looksLikeStatement) {
      ...
      if (noArticle.length >= 3) out.productName = noArticle;
    }
  }
  ...
  if (material) out.material = material[1].trim();

  // Quantità
  const qtyNum = t.match(/\b(\d{1,2})\s*(?:pz|pezzi|unità|unita|quantità|quantita)\b/);
  if (qtyNum) out.quantity = parseInt(qtyNum[1]);
  ...
  if (range) {
    ...
    if (single) out.budgetMax = parseInt(single[1]);
  }

  // Urgenza
  if (/(subito|oggi|immediato)/.test(t)) out.urgencyLevel = 'immediate';
  else if (/(24\s*h|domani)/.test(t)) out.urgencyLevel = '24h';
  else if (/(48\s*h|dopodomani)/.test(t)) out.urgencyLevel = '48h';
  else if (/(pochi giorni|entro la settimana|qualche giorno)/.test(t)) out.urgencyLevel = 'few_days';

  // Consegna
  if (/(ritiro|passo io|vengo io)/.test(t)) out.deliveryPreference = out.deliveryPreference === 'delivery' ? 'both' : 'pickup';
  if (/(consegna|a domicilio|spedizione)/.test(t)) out.deliveryPreference = out.deliveryPreference === 'pickup' ? 'both' : 'delivery';
  ...
  if (radius) out.actionRadius = Math.max(1, Math.min(100, parseInt(radius[1])));

  return out;
}

export function buildRequestDraft(collected: Partial<ClementeCollected>): any {
  ...
  const titleParts = [baseTitle || undefined].filter(Boolean) as string[];
  const title = titleParts.join(' ').trim() || 'Richiesta Prodotto';

  const bullets: string[] = [];
  if (collected.compatibility) bullets.push(`Compatibilità: ${collected.compatibility}`);
  if (collected.expectations) bullets.push(`Aspettative: ${collected.expectations}`);
  if (collected.technicalSpecs) bullets.push(`Specifiche: ${collected.technicalSpecs}`);
  if (collected.color) bullets.push(`Colore: ${collected.color}`);
  if (collected.size) bullets.push(`Taglia/Misure: ${collected.size}`);
  if (collected.material) bullets.push(`Materiale: ${collected.material}`);
  if (collected.quantity) bullets.push(`Quantità: ${collected.quantity}`);
