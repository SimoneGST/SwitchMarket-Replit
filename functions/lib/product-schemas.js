...
exports.findBestProductSchema = findBestProductSchema;
exports.generateSmartQuestion = generateSmartQuestion;
exports.formatQuestionForField = formatQuestionForField;
exports.collectAttributesFromSchema = collectAttributesFromSchema;
...
function findBestProductSchema(userInput) {
    ...
    for (const [category, keywords] of Object.entries(categoryMappings)) {
        if (keywords.some((kw) => input.includes(kw))) {
            return (exports.productSchemas.find((s) => s.category === category || s.subcategory === category || s.name.toLowerCase().includes(category)) || null);
        }
    }
    return null;
}
function generateSmartQuestion(schema, collectedData) {
    if (!schema)
        return null;
    const missingRequired = schema.fields.filter((f) => f.required && !collectedData?.[f.key]);
    if (missingRequired.length)
        return formatQuestionForField(missingRequired[0]);
    const missingOptional = schema.fields.filter((f) => !f.required && !collectedData?.[f.key]);
    if (missingOptional.length)
        return formatQuestionForField(missingOptional[0]);
    return null;
}
function formatQuestionForField(field) {
    switch (field.key) {
...
function collectAttributesFromSchema(schema, collected) {
    if (!schema)
        return undefined;
    const attrs = {};
