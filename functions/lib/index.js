...
app.post("/api/merchant/verify", authenticateUser, requireVerified, async (req, res) => {
    try {
        ...
        const filteredPayload = Object.fromEntries(Object.entries(basePayload).filter(([_, v]) => v !== undefined));
        if (!userDoc.exists) {
            await userRef.set({
                ...filteredPayload,
...
app.post("/api/assist/parse", async (req, res) => {
    try {
        ...
        if (allowedFields.includes('name')) {
            ...
            if (nameByLabel?.[1])
                updates.name = nameByLabel[1].trim();
        }
        // Description
        ...
        if (allowedFields.includes('category')) {
            ...
            for (const c of categories) {
                if (new RegExp(`\\b${c.replace(/ /g, '\\s*')}\\b`, 'i').test(p)) {
                    updates.category = c;
                    break;
                }
            }
            const catByLabel = p.match(/\b(?:categoria)\b\s*[:\-]?\s*([^\n,\.]{3,40})/i);
            if (!updates.category && catByLabel?.[1])
                updates.category = catByLabel[1].trim();
        }
        // Subcategory
        if (allowedFields.includes('subcategory')) {
            const subByLabel = p.match(/\b(?:sottocategoria)\b\s*[:\-]?\s*([^\n,\.]{3,40})/i);
            if (subByLabel?.[1])
                updates.subcategory = subByLabel[1].trim();
        }
        ...
        if (allowedFields.includes('condition')) {
            const map = { 'nuovo': 'new', 'nuova': 'new', 'eccellente': 'excellent', 'ottimo': 'excellent', 'buono': 'good', 'discreto': 'fair', 'da riparare': 'poor', 'rotto': 'poor' };
            for (const k of Object.keys(map)) {
                if (new RegExp(`\b${k}\b`, 'i').test(p)) {
                    updates.condition = map[k];
                    break;
                }
            }
        ...
        if (allowedFields.includes('brand')) {
            ...
            if (brandMatch?.[1])
                updates.brand = brandMatch[1].trim();
        }
...
app.post("/api/clemente/chat", async (req, res) => {
    try {
        const { message, context, attachedFile } = req.body || {};
        const text = typeof message === 'string' ? message.trim() : '';
        const debug = { path: 'clemente/chat', ts: Date.now(), ai: { keyPresent: !!(process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY), usedForAttachment: false, usedForReply: false }, attachment: null, notes: [] };
        ...
        if (attachedFile && (attachedFile.base64 || attachedFile.data || attachedFile.url)) {
            try {
                ...
                let mime = attachedFile.mimeType || attachedFile.type;
                let b64 = attachedFile.base64 || attachedFile.data;
...
Rispondi SOLO in JSON con campi opzionali: {"productName","brand","model","color","size","material","technicalSpecs","compatibility","expectations","budgetMin","budgetMax"}.`;
        ...
        if (!collected.category) {
            ...
            if (guessed)
                collected.category = guessed;
            debug.categoryGuessed = guessed || null;
        }
        const needed = [];
        if (!collected.productName && !collected.title)
            needed.push({ key: 'productName', prompt: 'Ok, di che prodotto parliamo esattamente? (es. scarpe da spinning con chiusura BOA)' });
        if (collected.budgetMin == null || collected.budgetMax == null)
            needed.push({ key: 'budget', prompt: 'Hai un budget indicativo? (es. 50-150€)' });
        if (!collected.location)
            needed.push({ key: 'location', prompt: 'In che città/area vuoi cercare?' });
        ...
        if (activeSchema) {
            try {
                const required = activeSchema.fields?.filter((f) => f.required) || [];
                const missingRequired = required.filter((f) => collected[f.key] == null || collected[f.key] === '');
                if (missingRequired.length) {
                    reply = formatQuestionForField(missingRequired[0]);
                }
                else {
                    reply = generateSmartQuestion(activeSchema, collected) || undefined;
                }
            }
            catch {
                reply = generateSmartQuestion(activeSchema, collected) || undefined;
        ...
        try {
            ...
            if (apiKey) {
                ...
                const conv = Array.isArray(req.body?.conversationHistory)
                    ? req.body.conversationHistory
                    : (Array.isArray(context?.conversationHistory) ? context.conversationHistory : []);
                const lastTurns = conv.slice(-12).map((m) => `${m?.isAI ? 'Clemente' : 'Cliente'}: ${String(m?.content || '').trim()}`).join('\n');
...
Non aggiungere testo fuori dal JSON. Evita ripetizioni e non richiedere campi già forniti.`;
                ...
                if (parsed?.assistantMessage)
                    reply = String(parsed.assistantMessage);
                ...
                if (aiRequestDraft && outSchema) {
                    const attrs = collectAttributesFromSchema(outSchema, { ...collected, ...aiRequestDraft }) || {};
                    // Auto-fill common fields from collected
                    const extras = {};
                    if (collected.size && attrs.size == null)
                        extras.size = collected.size;
                    if (collected.material && attrs.material == null)
                        extras.material = collected.material;
                    if (collected.color && attrs.color == null)
                        extras.color = collected.color;
                    // Domain-specific normalization for Abbigliamento
                    try {
                        const schemaCat = String(outSchema?.category || '').toLowerCase();
                        if (schemaCat === 'abbigliamento') {
                            // Guess type from text
                            const t = (text || '').toLowerCase();
                            if (!attrs.type) {
                                if (/maglietta|t\s*-?\s*shirt/.test(t))
                                    extras.type = 'maglietta';
                                else if (/camicia/.test(t))
                                    extras.type = 'camicia';
                                else if (/felpa/.test(t))
                                    extras.type = 'felpa';
                                else if (/pantaloncini|short/.test(t))
                                    extras.type = 'pantaloni';
                            }
                            // Gender default if required
                            if (!attrs.gender)
                                extras.gender = 'unisex';
                            // Color canonicalization (map to schema options)
                            const colorMap = { 'blu scuro': 'blu', 'azzurro': 'blu', 'nero': 'nero', 'bianco': 'bianco', 'grigio': 'grigio', 'verde': 'verde', 'rosso': 'rosso', 'marrone': 'marrone' };
                            const col = (extras.color || attrs.color || collected.color);
                            ...
                            const hasBudgetField = Array.isArray(outSchema.fields) && outSchema.fields.some((f) => f.key === 'budget');
                            if (hasBudgetField && !attrs.budget) {
                                const bmin = aiRequestDraft.budgetMin ?? collected.budgetMin;
                                const bmax = aiRequestDraft.budgetMax ?? collected.budgetMax;
                                if (typeof bmin === 'number' && typeof bmax === 'number')
                                    extras.budget = `${bmin}-${bmax}€`;
                                else if (typeof bmax === 'number')
                                    extras.budget = `0-${bmax}€`;
                            }
                        }
                    }
                    catch { }
                    ...
                    if (Object.keys(mergedAttrs).length)
                        aiRequestDraft.attributes = mergedAttrs;
                }
        ...
        const requestDraft = (() => {
            ...
            try {
                const schemaCat = String(activeSchema?.category || '').toLowerCase();
                if (schemaCat === 'abbigliamento') {
                    const t = (text || '').toLowerCase();
                    if (!attrs.type) {
                        if (/maglietta|t\s*-?\s*shirt/.test(t))
                    ...
                    const hasBudgetField = Array.isArray(activeSchema?.fields) && activeSchema.fields.some((f) => f.key === 'budget');
                    if (hasBudgetField && !attrs.budget) {
                        const bmin = draft.budgetMin ?? collected.budgetMin;
                        const bmax = draft.budgetMax ?? collected.budgetMax;
                        if (typeof bmin === 'number' && typeof bmax === 'number')
                            extras.budget = `${bmin}-${bmax}€`;
                        else if (typeof bmax === 'number')
                            extras.budget = `0-${bmax}€`;
        ...
        const ready = Object.keys(collected).some(k => ['productName', 'brand', 'model'].includes(k)) && ((collected.compatibility || collected.technicalSpecs || collected.expectations)) && (collected.budgetMin != null || collected.budgetMax != null || collected.urgencyLevel || collected.location);
        ...
        return res.status(500).json({ response: 'Mi dispiace, ho avuto un problema tecnico. Puoi riprovare?', message: 'Mi dispiace, ho avuto un problema tecnico. Puoi riprovare?', debug: { path: 'clemente/chat', ts: Date.now(), error: String(error?.message || error) } });
    }
});
    ...
    try {
        ...
        if (apiKey) {
            try {
                ...
                const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' });
                const { CLEMENTE_GUIDELINES } = await import('./clemente-guidelines.js');
                const convo = conversationHistory.map((m) => `${m?.role === 'assistant' ? 'Clemente' : 'Cliente'}: ${String(m?.content || '').trim()}`).join('\n');
                const reqSchema = `Rispondi SOLO in JSON valido con schema:
{
  ...
  "technicalSpecs": string|null
        ...
        const attrs = collectAttributesFromSchema(schema, extracted);
...
app.post("/api/leonardo/chat", async (req, res) => {
    try {
        const { GoogleGenerativeAI } = await import('@google/generative-ai');
    ...
    Rispondi in massimo 2 frasi quando possibile.`;
        ...
        const text = result?.response?.text?.() || 'Sono pronto ad aiutarti con il tuo profilo e la vetrina.';
...
app.get("/api/copilot/config", async (req, res) => {
    try {
...
app.put("/api/copilot/config", async (req, res) => {
    try {
        const userId = req.user.uid;
        const ref = admin.firestore().collection('copilotConfigs').doc(userId);
        await ref.set({
...
app.post("/api/suppliers/search", async (req, res) => {
    try {
        const { category = "", subcategory = "", quantity = 1, budgetPerUnit, frequency = "one_off", deliveryBy, location = {}, certifications = [], paymentTerms, notes = "", } = req.body || {};
        ...
        const results = catalog
            .filter(s => (!cat || s.categories.some(c => c.toLowerCase().includes(cat))) &&
...
// Request routes
app.post("/api/requests", authenticateUser, requireVerified, async (req, res) => {
    try {
        const body = req.body || {};
...
app.get("/api/requests", async (req, res) => {
    try {
        ...
        const all = snapshot.docs.map(doc => {
            const data = doc.data();
            const toISO = (ts) => ts?.toDate?.() ? ts.toDate().toISOString() : (typeof ts === 'string' ? ts : undefined);
            return {
                id: doc.id,
                ...data,
        ...
        const openOnly = all.filter((r) => (r.status || 'open') === 'open').slice(0, 50);
        res.json(openOnly);
    }
    catch (error) {
...
app.put('/api/requests/:requestId', authenticateUser, async (req, res) => {
    try {
        ...
        if (data?.buyerId !== req.user.uid)
            return res.status(403).json({ error: 'FORBIDDEN' });
        const allowed = {};
        const body = req.body || {};
        const copyIf = (k, v) => { if (v !== undefined)
            allowed[k] = v; };
...
app.delete('/api/requests/:requestId', authenticateUser, async (req, res) => {
    try {
        const { requestId } = req.params;
        const ref = admin.firestore().collection('requests').doc(requestId);
        const snap = await ref.get();
...
app.get("/api/requests/my", authenticateUser, async (req, res) => {
    try {
...
// Offer routes
app.post("/api/offers", authenticateUser, requireVerified, async (req, res) => {
    try {
                ...
                if (buyerId) {
                    ...
                    if (conversationId) {
                        ...
                        const existingOfferMsg = recentSnap.docs
                            ...
                            .find((m) => m.messageType === 'offer' && m.senderId === sellerId);
                        const messageData = {
                            senderId: sellerId,
                            messageType: 'offer',
    ...
    try {
        ...
        for (const r of requests) {
            const offSnap = await admin.firestore().collection('offers')
                ...
                .limit(20)
                .get();
            const offers = offSnap.docs.map(d => ({ id: d.id, ...d.data(), requestId: r.id, requestTitle: r.title || 'Richiesta' }));
...
app.post('/api/offers/:offerId/accept', authenticateUser, async (req, res) => {
    try {
...
app.post('/api/offers/:offerId/reject', authenticateUser, async (req, res) => {
    try {
        const { offerId } = req.params;
        const offerRef = admin.firestore().collection('offers').doc(offerId);
        const offerSnap = await offerRef.get();
...
app.get("/api/conversations", async (req, res) => {
    try {
        ...
        for (const convRaw of conversations) {
            ...
            if (!current || ts >= currTs)
                map.set(key, conv);
        }
        conversations = Array.from(map.values());
        // Enrich with participant photos
        const userIds = Array.from(new Set(conversations.flatMap((c) => [c.buyerId, c.sellerId]).filter(Boolean)));
...
app.get('/api/conversations/merchant', async (req, res) => {
    try {
        ...
        // Enrich with participant photos
        const userIds = Array.from(new Set(list.flatMap((c) => [c.buyerId, c.sellerId]).filter(Boolean)));
        const userDocs = await Promise.all(userIds.map(id => admin.firestore().collection('users').doc(id).get()));
        const usersById = new Map();
        userDocs.forEach(doc => usersById.set(doc.id, doc.data()));
...
app.get('/api/conversations/my', async (req, res) => {
    try {
...
app.post('/api/products', authenticateUser, requireVerified, async (req, res) => {
    try {
        const data = {
            ...req.body,
...
app.put('/api/products/:productId', authenticateUser, requireVerified, async (req, res) => {
    try {
...
app.delete('/api/products/:productId', authenticateUser, requireVerified, async (req, res) => {
    try {
        const { productId } = req.params;
...
app.all('/api/products/:productId/toggle', authenticateUser, requireVerified, async (req, res) => {
    try {
        const { productId } = req.params;
        const ref = admin.firestore().collection('products').doc(productId);
        const snap = await ref.get();
...
// Copilot toggle
app.post('/api/copilot/toggle', async (req, res) => {
    try {
...
exports.api = functions.https.onRequest({ secrets: [GEMINI_API_KEY, GOOGLE_API_KEY] }, app);
